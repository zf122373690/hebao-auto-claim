...
const codeFrom = (s: string, digits?: number) => { if (!/(验证码|动态码|校验码|随机码|口令|verification\s*code)/i.test(s)) return null; const m=s.match(new RegExp(`\\b\\d{${digits ?? 4},${digits ?? 8}}\\b`)); return m?.[0] ?? null; };
...
export default { async fetch(req: Request, env: Env): Promise<Response> {
  ...
  if (path === "/api/accounts" && req.method === "GET") return json({accounts: await env.DB.prepare("SELECT phone,label,level,last_login,(sess_key != '') logged_in FROM accounts ORDER BY created_at").all()});
  ...
  if ((path === "/api/hooks" || path === "/webhook/sms") && req.method === "POST") { if(env.WEBHOOK_SECRET && req.headers.get("x-webhook-secret") !== env.WEBHOOK_SECRET) return json({ok:false},401); const raw=await req.text(); let parsed: unknown=raw; try { parsed=raw ? JSON.parse(raw) : raw; } catch {} const text=typeof parsed === "object" && parsed && "text" in parsed ? String((parsed as any).text) : String(parsed); const code=codeFrom(text); const now=Date.now(); await env.DB.prepare("INSERT INTO sms_messages(body,code,phone,digits,received_at,expires_at) VALUES(?,?,?,?,?,?)").bind(text,code,phoneFrom(text),code?.length ?? null,now,now+10*60*1000).run(); return json({ok:true,code}); }
