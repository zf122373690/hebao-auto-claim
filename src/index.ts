...
export default { async fetch(req: Request, env: Env): Promise<Response> {
  ...
  if ((path === "/api/test-login" || path === "/api/test-claim") && req.method === "POST") { const b=await body(req), phone=String(b.phone??""); if(!/^\d{11}$/.test(phone)) return json({ok:false,msg:"手机号格式不正确"},400); return env.CLAIM_RUNNER.get(env.CLAIM_RUNNER.idFromName(phone)).fetch(`https://runner${path.replace("/api/test", "/test")}`,{method:"POST",body:JSON.stringify(b)}); }
